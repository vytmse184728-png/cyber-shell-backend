...
```bash
curl -i -X POST http://127.0.0.1:60081/api/terminal-events \
...
- `HTTP_TOOL_TARGET_URL`: single fixed base URL used by the HTTP tool
- `HTTP_TOOL_ALLOWED_HOSTS`: allowlist for Host header values that the model may use
- `HTTP_TOOL_ALLOWED_METHODS`: allowlist for HTTP methods the model may use
- `HTTP_TOOL_TIMEOUT_SECONDS`: timeout for the HTTP tool in seconds

## HTTP tool behavior

The HTTP tool no longer accepts a free-form destination URL from the model.

Instead:

- the backend always sends requests to the single fixed base URL from `HTTP_TOOL_TARGET_URL`
- the model may customize `method`, `path`, `query_params`, `headers`, and `body`
- the model may choose only an allowlisted `Host` header from `HTTP_TOOL_ALLOWED_HOSTS`
- attempts to override the destination URL or the `Host` header through normal headers are ignored server-side

This keeps the destination fixed in code while still allowing richer request verification flows such as POST bodies, custom headers, or custom paths.
